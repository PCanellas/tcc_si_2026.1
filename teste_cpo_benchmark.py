...
def construir_fases_de_busca(modelo, tasks, dados_tarefas):
    ...
    ordem = sorted(
        range(qtdTasks),
        key=lambda i: (span_total(dados_tarefas[i]), dados_tarefas[i][0] + dados_tarefas[i][1], dados_tarefas[i][2]),
        reverse=True,
    )

    fases = [modelo.search_phase([tasks[i], tasks[i + qtdTasks]]) for i in ordem]
...
def resolver_instancia(dados_tarefas, nome_instancia, tempo_limite=900):
    ...
    for i in range(qtdTasks):
        ...
        modelo.add(modelo.end_before_start(tasks[i], tasks[i+qtdTasks], delay=delay))
    ...
    if resp:
        ...
        resultado['gap'] = abs(resultado['valor_objetivo'] - resultado['melhor_bound']) / resultado['valor_objetivo'] * 100 if resultado['valor_objetivo'] > 0 else 0
    else:
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark CTSP com CP Optimizer")
    ...
    parser.add_argument('--time-limit', '-t', type=int, default=900, help='Tempo limite por instância (s)')
    parser.add_argument('--out', '-o', default='resultados_benchmark.txt', help='Arquivo de saída')
    ...
    for padrao in padroes:
        ...
        for arquivo in arquivos:
            ...
            try:
                ...
                resultado = resolver_instancia(dados_tarefas, nome_arquivo, tempo_limite=args.time_limit)
                resultados.append(resultado)
                
                # Mostrar progresso
                if resultado['solucao_encontrada']:
                    print(f"  ✓ Resolvido - Obj: {resultado['valor_objetivo']}, Bound: {resultado['melhor_bound']}, Tempo: {resultado['tempo_execucao']:.2f}s")
                else:
                    print(f"  ✗ Sem solução - Tempo: {resultado['tempo_execucao']:.2f}s")
    ...
    with open(nome_arquivo_resultado, 'w') as arquivo:
        ...
        arquivo.write("=" * 60 + "\n")
        arquivo.write(f"Tempo limite por instancia: {args.time_limit} segundos\n")
        arquivo.write(f"Total de instancias processadas: {len(resultados)}\n\n")

        # Cabeçalho da tabela
        arquivo.write(f"{'Instancia':<20} {'Tarefas':<8} {'Tempo(s)':<10} {'Objetivo':<12} {'Bound':<12} {'Gap(%)':<8} {'Status':<10}\n")
        arquivo.write("-" * 90 + "\n")
        ...
        for resultado in resultados:
            ...
            arquivo.write(f"{instancia:<20} {num_tarefas:<8} {tempo:<10} {objetivo:<12} {bound:<12} {gap:<8} {status:<10}\n")
        ...
        arquivo.write(f"Instancias resolvidas: {len(resolvidos)}/{len(resultados)} ({len(resolvidos)/len(resultados)*100:.1f}%)\n")
        arquivo.write(f"Instancias nao resolvidas: {len(nao_resolvidos)}\n")
        
        if resolvidos:
            tempo_medio = sum(r['tempo_execucao'] for r in resolvidos) / len(resolvidos)
